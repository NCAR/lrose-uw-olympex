...
$RAW_DIR = "/home/disk/bob/olympex/cfradial/moments/xband_dbz/rhi_single";
$TMP_DIR = $RAW_DIR."/tmp";
...
for $idate (0..$#dates) {
	...
	print "file = $file\n";
	($date,$time,$angle) = ($file =~ /cfrad\.(.*)_(.*)_xband_DBZ_(.*)_RHI\.nc/);
	...
	if ($angle > $last_angle) {
	    ...
	    $command = "$BIN_DIR/RadxConvert -v -params $PARAM_DIR/RadxConvert.xband.RHI -f $TMP_DIR/*.nc";
	    print "   command = $command\n";
	    system($command);
