...
$RAW_DIR = "/home/disk/bob/olympex/raw/d3r_qc/ku/sur";
$TMP_DIR = $RAW_DIR."/tmp";
$PARAM_DIR = "~meso/projDir/ingest/params";
#$BIN_DIR = "/home/disk/meso-home/meso/cvs/bin";
$BIN_DIR = "/home/disk/blitz/bin";
$OUT_DIR = "/home/disk/bob/olympex/cfradial/moments/d3r_ku_qc/sur";
  
@dates = ("20151110")
#@dates = ("20151109","20151110",
...
for $idate (0..$#dates) {
    ...
    $last_sweep = 0;
    $first_file = 1;
    foreach $file (<olympex_d3r_ku_*.nc>) {
	print "file = $file\n";
	($yyyymmdd,$hhmmss,$sweep) = ($file =~ /olympex_d3r_ku_(.*)_(.*)_(.*)\.nc/);
	...
	if ($sweep > $last_sweep) {
	    ...
	    #$command = "$BIN_DIR/RadxConvert -params $PARAM_DIR/RadxConvert.d3r.ku.archive -start \'$start_time\' -end \'$end_time\'";
	    $command = "$BIN_DIR/RadxConvert -params $PARAM_DIR/RadxConvert.d3r.ku.archive.sur -f $TMP_DIR/*.nc";
